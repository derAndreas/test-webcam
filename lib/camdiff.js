...
class CamDiff extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // parse the options
    this.srcVideo = this.optionsHas(options, 'srcVideo', 'Need a the video element as source');
    this.motionCanvas = this.optionsHas(options, 'motion', 'Need a the motion element as dest');
    ...
    this.showMotionBox = options.showMotionBox || false;
    this.showMotionPx = options.showMotionPx || false;
    ...
    this.motionCanvas = this._setupCanvas(this.diffWidth, this.diffHeight, this.motionCanvas);
    this.motionCanvasCtx = this.motionCanvas.getContext('2d');
  ...
  createDiff(diffData) {
    ...
    let score = 0;
    let motionPixels = this.showMotionPx ? [] : null;
    let motionBox = null;
    let i = 0;
    let len = rgba.length;
    let pxDiff;
